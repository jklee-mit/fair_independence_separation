...
print("Training networks with chi-squared penatly...")
...
for j in range(num_lambdas):
    ...
    for kk in range(30):
        ...
        net2 = ScalarNet()
        net3 = ScalarNet()
        optimizer1 = optim.SGD(net1.parameters(), lr=0.005, momentum=0.9)
...
for j in range(num_lambdas):
    ...
    tmp4 = []
    for kk in range(30):
        net1 = nn.Sequential(SimpleNet1(),SimpleNet2())
        net2 = ScalarNet()
        net3 = ScalarNet2()
        optimizer1 = optim.SGD(net1.parameters(), lr=0.005, momentum=0.9)
        optimizer2 = optim.SGD(net2.parameters(), lr=0.1, momentum=0.9)
        ...
        for epoch in range(60):  # loop over the dataset multiple times
            for iii, (data1,data2,data3) in enumerate(dataset_loader):
                ...
                # forward + backward + optimize
                loss = criteria_mse(net1(data1).flatten(), data2) - lambdas[j]*criteria(nn.Sequential(net1[0],net3), net2, data1, data3.view(-1,1))
                loss.backward()
                optimizer1.step()
                ...
                for sub_epoch in range(5):
                    ...
                    optimizer2.zero_grad()
                    optimizer3.zero_grad()
                    loss = criteria(nn.Sequential(net1[0],net3), net2, data1, data3.view(-1,1))
                    loss.backward()
                    optimizer2.step() 
                    optimizer3.step() 
...
#sio.savemat("output_ind.mat", {"all_res_double_reg_y":all_res_double_reg_y,
