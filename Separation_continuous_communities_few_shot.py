"""Soft-HGR for separation in the few-shot setting
"""
...
for j in range(num_lambdas):
    ...
    for kk in range(30):
        ...
        net3 = ScalarNet()
        net4 = ScalarNet()
        optimizer1 = optim.SGD(net1.parameters(), lr=3e-3, momentum=0.9)
        ...
        for epoch in range(100):  # loop over the dataset multiple times
            for iii, (data1,data2,data3) in enumerate(dataset_loader):
                ...
                loss = criteria_mse(net1(data1).flatten(), data2)
                loss.backward()
                optimizer1.step()
...
for j in range(num_lambdas):
    ...
    for kk in range(30):
        ...
        for epoch in range(5):  # loop over the dataset multiple times
            ...
            # forward + backward + optimize
            loss = criteria_mse(net1(data1).flatten(), data2) - lambdas[j]*(criteria(nn.Sequential(net1,net3), net2, data1, data_comb)**2 - criteria(nn.Sequential(net1,net3), net4, data1, data3.view(-1,1))**2)
            loss.backward()
            optimizer1.step()
            ...
            for sub_epoch in range(5):
                ...
                optimizer3.zero_grad()
                optimizer4.zero_grad()
                loss = criteria(nn.Sequential(net1,net3), net2, data1, data_comb)**2 - criteria(nn.Sequential(net1,net3), net4, data1, data3.view(-1,1))**2
                loss.backward()
                optimizer2.step() 
                optimizer3.step() 
                optimizer4.step() 
...
#sio.savemat("output_few.mat", {"all_res_double_reg_d":all_res_double_reg_d,
